...
import imageIV from "../../../../../../Images/PetOwner/Preventive.png";
...
import { ToastAddPhysicalConsultation, ToastSuccess, ToastDelete, ToastServicesUpdate, ToastAdd } from "../../../../../../Components/Toast";
...
const PreControlStart = (props) => {
  ...
  useEffect(async () => {
    ...
    setuser(userData);
    Axios.get(`${hostUrl}/preventiveControls/${userData.vetid}`).then((response) => {
  ...
  function reloadPetControl() {
    Axios.get(`${hostUrl}/preventiveControls/${user.vetid}`).then((response) => {
      setpreventiveControl(response.data);
      // console.log(response.data)
    });
  }
    ...
    >
              ...
              <Form.Group controlId="exampleForm.SelectCustom">
                ...
                >
                  <Form.Select
                    custom
                    required
                    defaultValue={"Preventive Control"}
                  // onSubmit={(e) => {
                  //   e.preventDefault();
                  //   setCategory('Consultation');
                  // }}
                  >
                    <option value="Preventive Control">Preventive Control</option>
                  </Form.Select>
                </FloatingLabel>
              </Form.Group>
      ...
      >
        <Popover id="popover-contained">
          ...
          <Popover.Body>
            <p>This table shows the preventive control services . </p>
          </Popover.Body>
        </Popover>
      </Overlay>
      <MaterialTable
        ...
        columns={columns}
        data={preventiveControl}
        title={"Preventive Controls Services"}
        cellEditable={false}
        options={{
          sorting: true,
        }}
...
export default PreControlStart;
